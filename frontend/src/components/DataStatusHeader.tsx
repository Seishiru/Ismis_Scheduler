...
export function DataStatusHeader({ lastUpdated }: DataStatusHeaderProps) {
  if (!lastUpdated) {
    return (
      <Card className="p-3 md:p-4 bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800 mb-4">
  ...
  return (
    <Card className={`p-3 md:p-4 mb-4 ${
      ...
      <div className="flex items-start gap-2">
        <CheckCircle className={`w-5 h-5 ${isRecent ? 'text-green-600 dark:text-green-500' : 'text-orange-600 dark:text-orange-500'} flex-shrink-0 mt-0.5`} />
        <p className={`text-xs md:text-sm ${isRecent ? 'text-green-700 dark:text-green-400' : 'text-orange-700 dark:text-orange-400'}`}>
